...
<script type="text/javascript">
	...
	*/
		...
		$.ajax({
			...
			success:function(data){
				if(data.status == 200){
					...
					$('#labOrder-data-datagrid').datagrid('clearSelections');
				}else{
	...
	/* 点击归还 */
	function ret(){
		var row = $("#labOrder-data-datagrid").datagrid("getSelected");
	...
	function disagree(){
		...
		var rstatus = row.rstatus;
		if(rstatus == 1){
			$.messager.alert('信息提示','尚未归还实验室,不能拒绝！','info');
			return;
		}
	...
	*/
	...
	function remove(){
		...
		var rstatus = row.rstatus;
		if(rstatus == 1){
			$.messager.alert('信息提示','尚未归还,不能删除！','info');
			return;
		}
